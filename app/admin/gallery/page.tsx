...
const ALL_TAGS = ["all", "hero", "exterior", "interior", "bedroom", "kitchen", "lifestyle", "detail", "gallery"];
...
export default function AdminGalleryPage() {
  ...
  const [uploadProperty, setUploadProperty] = useState("1");
  ...
  const filtered = photos.filter((p) => {
    ...
    const matchTag =
      filterTag === "all" || p.tags.includes(filterTag) || p.section === filterTag;
  ...
  const toggleFeatured = async (photo: Photo) => {
    ...
    setPhotos((prev) =>
      prev.map((p) => (p.id === photo.id ? { ...p, isFeatured: newValue } : p))
    );
    try {
      ...
      // revert on failure
      setPhotos((prev) =>
        prev.map((p) => (p.id === photo.id ? { ...p, isFeatured: photo.isFeatured } : p))
      );
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadProperty) formData.append("propertyId", uploadProperty);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-neutral-200 p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-ink/60">Tag/Section:</span>
      ...
      {loading ? (
        <div className="bg-white border border-neutral-200 py-20 text-center">
          <Loader2 size={32} className="mx-auto text-neutral-300 animate-spin mb-4" />
          <p className="text-ink/50 text-sm">Loading photos...</p>
        </div>
      ) : photos.length === 0 ? (
        <div className="bg-white border border-neutral-200 py-20 text-center">
          <Grid size={40} className="mx-auto text-neutral-300 mb-4" />
          <p className="text-ink/60 mb-2">No photos uploaded yet</p>
          <p className="text-sm text-ink/40 mb-6">
            Use the CMS page or click Upload Photos to add your first photo.
          </p>
          <Button variant="gold" onClick={() => setShowUpload(true)}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map((photo) => (
            ...
            >
              {/* Image */}
              <div
                className="cursor-pointer aspect-square overflow-hidden bg-neutral-100"
          ...
          <div className="bg-white w-full max-w-lg shadow-xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-neutral-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold"
                >
                  <option value="">No property (general gallery)</option>
              ...
              <div>
                ...
                >
                  ...
                  <option value="property-hero">Property Hero</option>
            ...
            <div className="flex gap-3 p-6 border-t border-neutral-100">
              <Button
                variant="outline"
                className="flex-1"
                onClick={() => { setShowUpload(false); setPendingFiles([]); }}
              >
                Cancel
              </Button>
              <Button
